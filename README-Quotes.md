...
## Dashboard Αποθέματος (§8β)

Σελίδα **PRICK → Inventory** (`/apothema`), προσβάσιμη και από τον ρόλο `warehouse`.

### Εγκατάσταση

Τρέξε **μία φορά** το [sql/stock_dashboard.sql](sql/stock_dashboard.sql) στη βάση
`OnlineData` — δημιουργεί μόνο τον πίνακα `StockReorderPoints` (όριο αναπαραγγελίας
ανά κωδικό+τύπο). Όλα τα υπόλοιπα μεγέθη υπολογίζονται live από τα υπάρχοντα
transactional δεδομένα, δεν αποθηκεύονται πουθενά:

| Στήλη | Υπολογισμός |
| --- | --- |
| Φυσικό (on_hand) | ελεύθερο υπόλοιπο `StockReceipts` + allocated τεμάχια μη απεσταλμένων παραγγελιών |
| Δεσμευμένο (committed) | ζήτηση ανοιχτών παραγγελιών γιατρών (Requested − Cancelled, order ≠ Fulfilled/Cancelled) — αυξάνεται με τη μετατροπή προσφοράς σε παραγγελία, μειώνεται με την αποστολή |
| Παραγγελμένα (on_order) | υπόλοιπο προς παραλαβή (Ordered − Received) από ανοιχτές παραγγελίες παραγωγής |
| Διαθέσιμο | Φυσικό − Δεσμευμένο (μπορεί να βγει αρνητικό) |
| Όριο (reorder_point) | επεξεργάσιμο inline στη σελίδα, αποθηκεύεται στο `StockReorderPoints` |
| Για παραγγελία | max(0, Όριο − (Διαθέσιμο + Παραγγελμένα)) |

- **Ομάδες:** Τρόφιμα = κωδικός που ξεκινά από `F` (case-insensitive), Λοιπά = τα υπόλοιπα.
- **Ένδειξη:** ⛔ κόκκινο όταν το διαθέσιμο ≤ 0 (με ζήτηση ή έλλειμμα), ⚠️ πορτοκαλί
  όταν χρειάζεται παραγγελία αλλά υπάρχει διαθέσιμο, «—» αλλιώς.
- **Stat tiles:** σύνολο ειδών, είδη προς παραγγελία, τεμάχια για παραγγελία,
  παραγγελμένα που αναμένονται.
- Η φόρμουλα ζει στο `Shared/Models/StockDashboardDtos.cs` (`StockDashboardLogic`) —
  κοινή για server/client — και καλύπτεται από `Tests/StockDashboardTests.cs`.
